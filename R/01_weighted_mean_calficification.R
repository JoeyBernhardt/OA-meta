...
lnRR_all <- read_csv("data-processed/calcification_lnRR_all.csv")
...
write_csv(overall_interaction_calcification, "data-processed/weighted_mean_calcification.csv")
...
overall_interaction_calcification %>% 
  ...
  geom_hline(yintercept = 0) + theme_bw() + ylab("weighted mean lnRR") + xlab("lnRR type")
ggsave("figures/weighted_mean_calcification.pdf")
