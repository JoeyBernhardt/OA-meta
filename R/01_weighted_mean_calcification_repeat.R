...
calc_ES_raw_calc <- read_csv("data-processed/calcification_repeat_lnRR_all.csv")
...
lnRR_all <- read_csv("data-processed/calcification_repeat_lnRR_all.csv")
...
Tau_sq_food <- (Q_food - df)/(sum(calcfood_1$term3) - (sum(calcfood_1$term3 ^ 2)/sum(calcfood_1$term3)))
...
write_csv(overall_interaction_calcification, "data-processed/weighted_mean_calcification_repeat.csv")
...
overall_interaction_calcification %>% 
  ...
  geom_hline(yintercept = 0) + theme_bw() + ylab("weighted mean lnRR") + xlab("lnRR type")
ggsave("figures/weighted_mean_calcification_repeat.pdf")
