...
calc_ES_raw_growth <- read_csv("data-processed/growth_repeat_lnRR_all.csv")
...
lnRR_all <- read_csv("data-processed/growth_repeat_lnRR_all.csv")
...
write_csv(overall_interaction2, "data-processed/weighted_mean_growth_repeat.csv")
...
overall_interaction2 %>% 
  ...
  geom_hline(yintercept = 0) + theme_bw() + ylab("weighted mean lnRR") + xlab("lnRR type")
ggsave("figures/weighted_mean_growth_repeat.pdf")
