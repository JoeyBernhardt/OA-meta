...
calcification <- read_csv("data-raw/calcification_exact.csv")
...
elevatedlow <- cal1  %>% 
  ...
  rename(sd_elevatedlow = sd_ambient) %>% 
...
elevatedhigh <- cal1  %>% 
  ...
  rename(sd_elevatedhigh = sd_ambient) %>% 
...
write_csv(lnRR_all, "data-processed/calcification_exact_lnRR_all.csv")
